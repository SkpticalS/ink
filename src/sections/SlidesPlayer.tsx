      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-btn text-xs transition-all flex items-center gap-1.5 ${
              spotlightActive
                ? "bg-gold-600 text-white"
                : "bg-xuan-aged text-ink-500 hover:bg-xuan-aged/80"
            }`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-2 space-y-2">
          {DEMO_SLIDES.map((slide, idx) => (
            <button
              ...
              className={`w-full text-left p-3 rounded-card transition-all duration-300 border-2 ${
                idx === currentIndex
                  ? "border-gold-600 bg-gold-100/30 shadow-md"
                  : "border-transparent bg-xuan-aged/30 hover:bg-xuan-aged/50"
              }`}
            >
              <div className="flex items-center gap-2">
                <span className={`text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center ${
                  idx === currentIndex ? "bg-gold-600 text-white" : "bg-xuan-aged text-ink-400"
                }`}>
                  {idx + 1}
                </span>
                <span className={`text-xs truncate ${idx === currentIndex ? "text-ink-900 font-medium" : "text-ink-500"}`}>
                  {slide.title}
                </span>
              </div>
            </button>
          ...
          <div className="pt-4 border-t border-gold-600/10">
            <div className="flex items-center gap-2 justify-center">
              <button
                ...
                className="w-8 h-8 rounded-full bg-xuan-aged flex items-center justify-center text-ink-500 hover:bg-gold-600 hover:text-white transition-all disabled:opacity-30"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg>
              </button>
              <button
                onClick={togglePlay}
                className="w-10 h-10 rounded-full bg-cinnabar text-white flex items-center justify-center hover:scale-110 transition-transform seal-btn"
              >
                {isPlaying ? (
            ...
            <div className="mt-3 h-1 bg-xuan-aged rounded-full overflow-hidden">
              <div className="h-full bg-gold-600 rounded-full transition-all" style={{ width: `${progress}%` }} />
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7">
          <div ref={slideRef} className="chinese-frame shadow-frame relative">
            <div className="chinese-frame-inner bg-xuan-white relative overflow-hidden min-h-[420px]">
              {/* Spotlight overlay */}
              {spotlightActive && (
                <div className="absolute inset-0 z-20 pointer-events-none">
                  ...
                  <div
                    ...
                    style={{
                      background: "radial-gradient(circle, transparent 30%, rgba(26,26,26,0.6) 70%)",
                      top: "30%",
                      left: "50%",
              ...
              <div className="p-8 md:p-12">
                ...
                <h2 className="font-title text-2xl md:text-3xl text-ink-900 tracking-wider mb-4">
                  {currentSlide.title}
                </h2>
                  ...
                  <ul className="space-y-3">
                    {currentSlide.bulletPoints.map((point, idx) => (
                      <li key={idx} className="flex items-start gap-3">
                        <span className="flex-shrink-0 w-6 h-6 rounded-full bg-gold-600/10 text-gold-600 flex items-center justify-center text-xs font-bold">
                          {idx + 1}
                        </span>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-xuan-white/90 rounded-card p-4 shadow-paper border border-gold-600/10">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {isPlaying ? (
                  <><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> 暂停语音</>
                ) : (
          ...
          <div className="bg-xuan-white/90 rounded-card p-4 shadow-paper border border-gold-600/10">
            ...
            <div className="space-y-2">
              <button className="w-full py-2 px-3 rounded-btn bg-xuan-aged/60 text-ink-600 text-xs hover:bg-xuan-aged transition-all flex items-center gap-2 border border-gold-600/10">
          ...
          {spotlightActive && (
            <div className="bg-gold-100/50 rounded-card p-3 border border-gold-600/15 text-center">
              <span className="text-xs text-gold-600 animate-pulse">● 聚光灯模式已开启</span>
            </div>
          )}
        </div>
      </div>
